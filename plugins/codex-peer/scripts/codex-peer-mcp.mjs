...
async function callTool(name, args) {
  ...
  if (name === "codex_peer_review") {
    ...
    const job = startJob({
      ...value,
      allow_writes: false,
      task: `${scope}${focus}\nBe read-only. Report concrete findings with severity, file/line evidence, and suggested fixes. Review the code and diff statically: do not run test suites, builds, linters, deploys, package installs, or other CI-equivalent commands; validation is the delegating agent's responsibility. If the diff includes user-facing copy (labels, messages, emails), also review it for spec-notation leakage (e.g. pipe/slash-delimited alternatives), internal jargon or key names surfacing in UI text, and terminology inconsistency with existing copy. Read only the diff under review and its direct dependencies: do not bulk-read files outside the review scope, and do not read repository-wide documents (AGENTS.md, Justfile, README) in full unless a specific finding requires it.`,
    });
